...
public class Exam1 {
    public static void main(String[] args) {
        ...
        // 1. 참새 타입에서 조류타입으로 타입변환이 될까?
        // 가능한 이유: 1. 상속관계(논리) 2. 참새인스턴스가 생성될때+조류+동물 인스턴스생성(물리)
        조류 brid2 = sparrow1; // 자동타입변환      / 업캐스팅(올라가기)
        // 2. 참새 타입 에서 동물 타입 으로 될까?
        동물 animal2 = sparrow1; // 업캐스팅
        // 3. 동물 타입에서 참새 타입으로 될까? 주의!!!!!!!!!!!!!!!!!
        // 주의할점: 1) 변환활타입명시 2) 변환활자료가 변환할타입 포함하는지?
        참새 sparrow2 = (참새)animal2; // 강제타입변환 / 다운캐스팅(내려가기)
        // 4. 동물 타입에서 참새 타입으로 될까? 주의!!!!!!!!!!!!!!!!!
        // 즉 자식이 태어날때 부모도 태어나지만 자식은 태어나지 않는다. 나랑
        // 참새 sparrow3 = (참새)animal1;  // 오류발생! 동물1 태어날때 참새가 생성되지 않았다.

        // 1~3 : 자료: 참새 , 타입:참새 --> 자료:참새 , 타입:조류 --> 자료:참새 , 타입:동물
        // --> 자료:참새 , 타입:조류
        // --> 자료:참새 , 타입:돔물
        // --> 자료:참새 , 타입 참새
