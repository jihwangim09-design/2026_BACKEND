...
class 상위클래스{
    int value1 = 10;
    int value2 = 20;
    상위클래스(){ System.out.println("상위가 탄생");}
...
class 하위클래스 extends 상위클래스{
    ...
    // 주의할점: 오버로딩 vs 오버라이딩
    void show( int a ){} // 부모랑 생성자랑 메소드명은 같은데 매개변수가 다르면 오버로딩

    @Override // 메소드 위에 @Override 명시, 빈칸에 컨트롤+스페이스바
    void show(){ // 상위클래스의 메소드 선언부가 모두 동일하면 오버라이딩(재정의)
