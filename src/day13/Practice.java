...
public class Practice {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            // 위치찾기 구현
            if( ch == 1 ){ 
                System.out.println("차량번호입력");
                String 차량번호 = scan.next();
                String[] rows = carParkingList.split("\n");
                boolean result = false;
                for (String row : rows) {
                    String[] col = row.split(",");
                    if (col[1].equals(차량번호)) {
                        System.out.println("위치: " + col[0]);
                        result = true;
                        break;
                    }
                }
            ...
            if( ch == 2 ){
                ...
                for ( String row : rows ) {
                    ...
                    if (col[0].equals(location+"")) { 
                        // col[0]은 String이고 location은 int라서 빈문자열을 더해 문자열로 바꿔서 비교
                        result = true;
                        break;
                    }
                }
                if (result) {System.out.println("중복된 위치 입니다.");}
                else {
