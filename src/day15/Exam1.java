...
public class Exam1 {
    public static void main(String[] args) {
        /*
            ...
            제네릭타입: 클래스 (정의)만들때 타입 정하지 않고 클래스 사용할때 타입 정한다.
                List<BoradDto> list = new ArrayList<>()
                - List 인터페이스를 (정의)만들떄 타입을 정하지 않고 List 인터페이스를 사용할 때 BoardDto라고 타입을 정했다.
                - 정의/만들기: class 클래스명 {} , 사용/호출: 클래스명 변수명;

            컬렉션프레임워크
                List 인터페이스:    중복허용, 인덱스(순서)있음, ArrayList , LinkedList , Vector 등
                Set 인터페이스:     중복불가, 인덱스(순서)없음, HashSet, TreeSet 등
                Map 인터페이스:     KEY중복불가/VALUE중복허용 , 인덱스(순서)없다, HashMap, TableMap, TreeMap 등
        */  
        // [1] Map : key와value 한쌍(엔트리)으로 여러개 쌍(엔트리) 저장하는 구조 = JSON( { } ) / DTO가변
        ...
        // 7) .clear( ): 모든 엔트리 삭제
        map.clear();
        // 8) .isEmpty( ): 엔트리가 1개도 없으면 true 아니면 false
        System.out.println(map.isEmpty());
        // * 활용처: 1] JSON <-- 통신 --> DTO/MAP(JAVA) 2] 자료구조(암호화) 등등
        // 9) 반복문 관계
